...
def stem(token_words):
    '''
        词形归一化
    '''
    wordnet_lemmatizer = WordNetLemmatizer()  # 单词转换原型
    words_lemmatizer = []
    for word, tag in token_words:
        if tag.startswith('NN'):
            word_lemmatizer = wordnet_lemmatizer.lemmatize(word, pos='n')  # n代表名词
        elif tag.startswith('VB'):
            word_lemmatizer = wordnet_lemmatizer.lemmatize(word, pos='v')  # v代表动词
        elif tag.startswith('JJ'):
            word_lemmatizer = wordnet_lemmatizer.lemmatize(word, pos='a')  # a代表形容词
        elif tag.startswith('R'):
            word_lemmatizer = wordnet_lemmatizer.lemmatize(word, pos='r')  # r代表代词
        else:
            word_lemmatizer = wordnet_lemmatizer.lemmatize(word)
        words_lemmatizer.append(word_lemmatizer)
    return words_lemmatizer
...
def to_lower(token_words):
    '''
        统一为小写
    '''
    words_lists = [x.lower() if not x.isupper() else x for x in token_words]
    return words_lists

def add_dash(token_words):
    words_lists = []
    for word in token_words:
        words_lists.append(word)
        if '-' in word:
            words_lists += word.split('-')
    return words_lists

def to_singular(token_words):
    words_lists = [inf.singularize(x) for x in token_words]
    return words_lists
...
def pre_process(text):
    ...
    token_words = add_dash(token_words)
    token_words = to_singular(token_words)
    return token_words
...
'多进程处理'
def pool_extract(data, f, vec_model, postpostfix, chunk_size, worker=4):
    ...
    while end < len_data:
        ...
        rslt = p.apply_async(f, (data[start:end], postpostfix, vec_model))
        start = end
        res.append(rslt)
    p.close()
    p.join()
